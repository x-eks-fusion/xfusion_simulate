...
class VisualGraphWindow(QMainWindow):
    ...
    def initialize_sidebar(self):
        ...
        sw.addComp(QApplication.translate("MainWindow", "函数"), self.func_tree, False,10)

        self.vari_tree = VariableTreeWidget(sw)
        sw.addComp(QApplication.translate("MainWindow",'变量'), self.vari_tree, False,20)
        ...
        sw.addComp(QApplication.translate("MainWindow",'模块库'), self.model_tree, False,10)

        self.detail_widget = DetailWidget(None, self)
        sw.addComp(QApplication.translate("MainWindow",'详情'), self.detail_widget, False,20)
    ...
    def action_triggered(self):
        ...
        self.menu_bar.connect(self.menu_bar.clearMenuAction, self.clear_recent_files)
        ...
        self.menu_bar.connect(self.menu_bar.selectAllAction, self.selectAllItems)
        self.menu_bar.connect(self.menu_bar.deselectAllAction, self.deselectAllItems)
        self.menu_bar.connect(self.menu_bar.showLeftSidebarAction, self.show_left)
        self.menu_bar.connect(self.menu_bar.showRightSidebarAction, self.show_right)
        self.menu_bar.connect(self.menu_bar.renderSelectedAction, self.renderSelected)
        self.menu_bar.connect(self.menu_bar.renderAllNodesAction, self.renderGraph)
        self.menu_bar.connect(self.menu_bar.alignVCenterAction, self.alignVCenter)
        self.menu_bar.connect(self.menu_bar.verticalDistributedAction, self.alignVDistributed)
        self.menu_bar.connect(self.menu_bar.alignHCenterAction, self.alignHCenter)
        self.menu_bar.connect(self.menu_bar.horizontalDistributedAction, self.alignHDistributed)
        ...
        self.menu_bar.connect(self.menu_bar.alignBottomAction, self.alignBottom)
        self.menu_bar.connect(self.menu_bar.straightenEdgeAction, self.straightenEdge)
        self.menu_bar.connect(self.menu_bar.runAction, self.run_graph)
        self.menu_bar.connect(self.menu_bar.runInBackAction, self.run_graph_in_back)
